...
const JoinLedger = () => {
  ...
  // Process invite code when component mounts
  useEffect(() => {
    if (inviteCode) {
      processInviteCode();
    }
      ...
      <View className="join-ledger-container">
        ...
        {success && ledgerInfo && (
          <View className="join-ledger-success">
            <Text className="join-ledger-success-title">
              成功加入账本
            </Text>
            <Text className="join-ledger-success-name">
              {ledgerInfo.name || '账本'}
            </Text>
            <Text className="join-ledger-success-desc">
              正在跳转到账本...
            </Text>
          </View>
        )}
