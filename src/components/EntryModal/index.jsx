...
const EntryModal = ({ visible, onClose, onSuccess, editEntry = null }) => {
  ...
  const handleNumberDelete = () => {
    setForm(prev => ({ ...prev, amount: prev.amount.length > 0 ? prev.amount.slice(0, -1) : '' }));
  };
      ...
      >
        <View className="entry-modal">
          ...
          <View className="entry-modal-categories">
            <View className="entry-modal-categories__title">分类</View>
            <View className="entry-modal-categories__list">
              {currentCategories.map((category) => (
                <View 
                  key={category}
                  className={`entry-modal-categories__item ${form.category === category ? 'entry-modal-categories__item--active' : ''}`}
